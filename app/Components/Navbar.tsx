  ...
  return (
    <nav className="py-4 px-[24px] sm:px-[40px] md:px-[50px] lg:px-[70px] h-[89px] flex items-center justify-between relative">
      <div className="relative w-[160px] sm:w-[180px] md:w-[200px] lg:w-[220px] h-[41px] flex-shrink-0">
        <Image src="/logo.png" alt="Logo" width={208.89} height={40} className="object-contain" />
      </div>
      ...
      <ul className="hidden lg:flex h-[41px] items-center gap-6 xl:gap-8 font-Poppins font-normal text-[14px] xl:text-[16px] text-white relative">
        ...
        <li><Link href="/case-studies">Marketplace</Link></li>
        <li><Link href="/case-studies">Case Studies</Link></li>
        <li><Link href="/blog">Blog</Link></li>
        <li><Link href="/faq">FAQ</Link></li>
        <button
          className="w-[121px] h-[41px] px-4 py-3 flex items-center justify-center gap-2 text-white text-[14px] font-semibold rounded-lg bg-white/25"
        >
          Get in Touch
        </button>
      </ul>
      {menuOpen && (
        <div className="fixed top-0 left-0 w-full h-screen bg-black bg-opacity-90 flex flex-col items-center justify-center space-y-6 transform transition-all duration-300 md:flex lg:hidden">
